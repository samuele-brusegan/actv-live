(function () {
    ...
    const more = [['/lines-map','Mappa linee'],['/line-schedule','Orari delle linee'],['/live-map','Bus in servizio'],['/delay-stats','Statistiche ritardi'],['/feedback','Invia feedback'],['/delete-cookie','Privacy e dati']];
    ...
    const menu = document.createElement('div'); menu.className = 'actv-more-menu'; menu.id = 'actv-more'; menu.setAttribute('aria-label','Altre funzioni'); menu.innerHTML = more.map(([href,label]) => `<a href="${href}" class="${href === '/feedback' ? 'feedback-menu-link' : ''}">${href === '/feedback' ? '✎ ' : ''}${label}</a>`).join(''); document.body.appendChild(menu);
